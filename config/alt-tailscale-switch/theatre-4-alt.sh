#!/usr/bin/env bash
# Alternative to GL-iNet A-1300 for Theatre 4 — generic Linux box (mini-PC or
...
# configs in ../gl-inet/.
cat > /etc/dnsmasq.d/theatre4.conf <<EOF
interface=lan0,lan1
dhcp-range=192.168.5.100,192.168.5.149,12h
dhcp-host=REPLACE_WITH_MAC_01,192.168.5.20   # birddog-play
dhcp-host=REPLACE_WITH_MAC_02,192.168.5.2    # atem-mini-extreme-iso
dhcp-host=REPLACE_WITH_MAC_03,192.168.5.5    # powerpoint-main
dhcp-host=REPLACE_WITH_MAC_04,192.168.5.6    # powerpoint-backup
dhcp-host=REPLACE_WITH_MAC_05,192.168.5.7    # vt-main
dhcp-host=REPLACE_WITH_MAC_06,192.168.5.8    # vt-backup
dhcp-host=REPLACE_WITH_MAC_07,192.168.5.10   # control-laptop
EOF
systemctl restart dnsmasq

# --- Tailscale — identical to the GL-iNet case, see ../tailscale-up-all-devices.sh
# tailscale up --advertise-routes=192.168.5.0/24 --accept-routes --advertise-tags=tag:theatre --hostname=theatre-4-router
