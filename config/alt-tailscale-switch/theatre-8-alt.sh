#!/usr/bin/env bash
# Alternative to GL-iNet A-1300 for Theatre 8 — generic Linux box (mini-PC or
...
# configs in ../gl-inet/.
cat > /etc/dnsmasq.d/theatre8.conf <<EOF
interface=lan0,lan1
dhcp-range=192.168.9.100,192.168.9.149,12h
dhcp-host=REPLACE_WITH_MAC_01,192.168.9.20   # birddog-play
dhcp-host=REPLACE_WITH_MAC_02,192.168.9.2    # atem-mini-extreme-iso
dhcp-host=REPLACE_WITH_MAC_03,192.168.9.5    # powerpoint-main
dhcp-host=REPLACE_WITH_MAC_04,192.168.9.6    # powerpoint-backup
dhcp-host=REPLACE_WITH_MAC_05,192.168.9.7    # vt-main
dhcp-host=REPLACE_WITH_MAC_06,192.168.9.8    # vt-backup
dhcp-host=REPLACE_WITH_MAC_07,192.168.9.10   # control-laptop
EOF
systemctl restart dnsmasq

# --- Tailscale — identical to the GL-iNet case, see ../tailscale-up-all-devices.sh
# tailscale up --advertise-routes=192.168.9.0/24 --accept-routes --advertise-tags=tag:theatre --hostname=theatre-8-router
